...
class DTFX:
    ...
    def ZTKG_CITY_TABLE(self, dsh):
        ...
        writer = pd.ExcelWriter(path + '\\' + ds_mc + "考生答题分析总体概括(理科综合).xlzh")

        # 理科
        df = pd.DataFrame(data=None, columns=['维度', '人数', '比率(%)', '平均分', '标准差', '差异系数', '平均分(全省)'])

        sql = r'select count(a.zh) from kscj as a right join jbxx as b on a.KSH = b.KSH ' \
              r'WHERE b.DS_H=%s and a.kl=1 and a.zh!=0'
        print(sql)
        ...
        sql = r"select count(A.zh) as num,AVG(A.zh) as mean,STDDEV_SAMP(A.zh) as std " \
              ...
              r"where b.DS_H=%s and b.XB_H = 1 and a.kl=1 and a.zh!=0"
        ...
        sql = r"select AVG(A.zh) as mean from kscj as A right join jbxx as B on A.KSH = B.KSH" \
        ...
        sql = r"select count(A.zh) as num,AVG(A.zh) as mean,STDDEV_SAMP(A.zh) as std " \
              r"from kscj as a right join jbxx as b on a.KSH = b.KSH " \
              r"where b.DS_H=%s and b.XB_H = 2 and a.kl=1 and a.zh!=0"
        ...
        sql = r"select count(A.zh) as num,AVG(A.zh) as mean,STDDEV_SAMP(A.zh) as std " \
              r"from kscj as a right join jbxx as b on a.KSH = b.KSH " \
              r"where b.DS_H=%s and (b.KSLB_H = 1 OR b.KSLB_H = 3) and a.kl=1 and a.zh!=0"
        ...
        sql = r"select count(A.zh) as num,AVG(A.zh) as mean,STDDEV_SAMP(A.zh) as std " \
              r"from kscj as a right join jbxx as b on a.KSH = b.KSH " \
              r"where b.DS_H=%s and (b.KSLB_H = 2 OR b.KSLB_H = 4) and a.kl=1 and a.zh!=0"
        ...
        sql = r"select count(A.zh) as num,AVG(A.zh) as mean,STDDEV_SAMP(A.zh) as std " \
              r"from kscj as a right join jbxx as b on a.KSH = b.KSH " \
              r"where b.DS_H=%s and (b.KSLB_H = 1 OR b.KSLB_H = 2) and a.kl=1 and a.zh!=0"
        ...
        sql = r"select count(A.zh) as num,AVG(A.zh) as mean,STDDEV_SAMP(A.zh) as std " \
              r"from kscj as a right join jbxx as b on a.KSH = b.KSH " \
              r"where b.DS_H=%s and (b.KSLB_H = 3 OR b.KSLB_H = 4) and a.kl=1 and a.zh!=0"
        ...
        sql = r"select AVG(A.zh) as mean from kscj as A right join jbxx as B " \
              r"on A.KSH = B.KSH and a.kl=1 where  a.zh!=0"
        self.cursor.execute(sql)
        result.append(self.cursor.fetchone()[0])
        ...
        sql = "select count(zh),AVG(A.zh) as mean,STDDEV_SAMP(A.zh) as std FROM kscj as A where A.kl=1 and a.zh!=0"
        self.cursor.execute(sql)
        result = self.cursor.fetchone()
        result = list(result)
        result.append(float(result[2]) / float(result[1]))  # 差异系数
        ...
        for xqh in xqhs:
            ...
            sql = "select count(zh),AVG(A.zh) as mean,STDDEV_SAMP(A.zh) as std FROM kscj as A " \
                  "right JOIN JBXX AS B ON A.KSH = B.KSH WHERE A.kl=1 and a.zh!=0 and B.XQ_H = " + xqh[0]
            self.cursor.execute(sql)
            result = self.cursor.fetchone()
            result = list(result)
            result.append(float(result[2]) / float(result[1]))  # 差异系数
    ...
    def ZTKG_PROVINCE_TABLE(self):
        ...
        # 性别
        for xb in xbs:
            sql = "select count(a.zh) as num,AVG(a.zh) as mean,STDDEV_SAMP(a.zh) as std " \
