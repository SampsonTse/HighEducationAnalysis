...
class DTFX:
    ...
    def ZTGK_CITY_TABLE(self, dsh):
        ...
        sql = r'select count(a.zh) from kscj  a right join JBXX  b on a.KSH = b.KSH WHERE b.DS_H=' + dsh + r' and a.kl=1 and a.zh!=0'
        ...
        sql = r"select count(A.zh)   num,AVG(A.zh)   mean,STDDEV_SAMP(A.zh)   std " \
              ...
              r" where b.DS_H=" + dsh + r" and b.XB_H = 1 and a.kl=1 and a.zh!=0"
        ...
        sql = r"select AVG(A.zh)   mean from kscj   A right join JBXX   B on A.KSH = B.KSH" \
              r" where b.XB_H = 1 and a.kl=1 and a.zh!=0"
        self.cursor.execute(sql)
        ...
        sql = r"select count(A.zh)   num,AVG(A.zh)   mean,STDDEV_SAMP(A.zh)   std " \
              r"from kscj   a right join JBXX   b on a.KSH = b.KSH " \
              r"where b.DS_H=" + dsh + r" and b.XB_H = 2 and a.kl=1 and a.zh!=0"
        ...
        sql = r"select AVG(A.zh)   mean from kscj   A right join JBXX   B on A.KSH = B.KSH " \
              r"where b.XB_H = 2 and a.kl=1 and a.zh!=0"
        self.cursor.execute(sql)
        result.append(self.cursor.fetchone()[0])
        ...
        sql = r"select count(A.zh)   num,AVG(A.zh)   mean,STDDEV_SAMP(A.zh)   std " \
              r"from kscj   a right join JBXX   b on a.KSH = b.KSH " \
              r"where b.DS_H=" + dsh + r" and (b.KSLB_H = 1 OR b.KSLB_H = 3) and a.kl=1 and a.zh!=0"
        ...
        sql = r"select AVG(A.zh)   mean from kscj   A right join JBXX   B on A.KSH = B.KSH " \
              r"where (b.KSLB_H = 1 OR b.KSLB_H = 3) and a.kl=1 and a.zh!=0"
        self.cursor.execute(sql)
        result.append(self.cursor.fetchone()[0])
        ...
        sql = r"select count(A.zh)   num,AVG(A.zh)   mean,STDDEV_SAMP(A.zh)   std " \
              r"from kscj   a right join JBXX   b on a.KSH = b.KSH " \
              r"where b.DS_H=" + dsh + r" and (b.KSLB_H = 2 OR b.KSLB_H = 4) and a.kl=1 and a.zh!=0"
        ...
        sql = r"select AVG(A.zh)   mean from kscj   A right join JBXX   B on A.KSH = B.KSH " \
              r"where (b.KSLB_H = 2 OR b.KSLB_H = 4) and a.kl=1 and a.zh!=0"
        self.cursor.execute(sql)
        result.append(self.cursor.fetchone()[0])
        ...
        sql = r"select count(A.zh)   num,AVG(A.zh)   mean,STDDEV_SAMP(A.zh)   std " \
              r"from kscj   a right join JBXX   b on a.KSH = b.KSH " \
              r"where b.DS_H=" + dsh + r" and (b.KSLB_H = 1 OR b.KSLB_H = 2) and a.kl=1 and a.zh!=0"
        ...
        sql = r"select AVG(A.zh)   mean from kscj   A right join JBXX   B on A.KSH = B.KSH " \
              r"where (b.KSLB_H = 1 OR b.KSLB_H = 2) and a.kl=1 and a.zh!=0"
        self.cursor.execute(sql)
        result.append(self.cursor.fetchone()[0])
        ...
        sql = r"select count(A.zh)   num,AVG(A.zh)   mean,STDDEV_SAMP(A.zh)   std " \
              r"from kscj   a right join JBXX   b on a.KSH = b.KSH " \
              r"where b.DS_H=" + dsh + r" and (b.KSLB_H = 3 OR b.KSLB_H = 4) and a.kl=1 and a.zh!=0"
        ...
        sql = r"select AVG(A.zh)   mean from kscj   A right join JBXX   B on A.KSH = B.KSH" \
              r" where (b.KSLB_H = 3 OR b.KSLB_H = 4) and a.kl=1 and a.zh!=0"
        self.cursor.execute(sql)
        result.append(self.cursor.fetchone()[0])
        ...
        sql = r"select count(A.zh)   num,AVG(A.zh)   mean,STDDEV_SAMP(A.zh)   std " \
              r"from kscj   a right join JBXX   b on a.KSH = b.KSH" \
              r" where b.DS_H=" + dsh + r" and a.kl=1 and a.zh!=0"
        ...
        sql = r"select AVG(A.zh)   mean from kscj   A right join JBXX   B on A.KSH = B.KSH and a.kl=1 where a.zh!=0"
        self.cursor.execute(sql)
        result.append(self.cursor.fetchone()[0])
        ...
        sql = "select count(zh),AVG(A.zh)   mean,STDDEV_SAMP(A.zh)   std FROM kscj   A where A.kl=1 and a.zh!=0"
        self.cursor.execute(sql)
        result = self.cursor.fetchone()
        result = list(result)
        result.append((float(result[2]) / float(result[1])) * 100)  # 差异系数
        ...
        sql = r"select count(zh),AVG(A.zh)   mean,STDDEV_SAMP(A.zh)   std FROM kscj   A " \
              r"where A.kl=1 and a.zh!=0 and A.KSH LIKE '" + dsh + r"%'"
        self.cursor.execute(sql)
        result = self.cursor.fetchone()
        result = list(result)
        result.append((float(result[2]) / float(result[1])) * 100)  # 差异系数
        ...
        for xqh in xqhs:
            ...
            sql = "select count(zh),AVG(A.zh)   mean,STDDEV_SAMP(A.zh)   std FROM kscj   A " \
                  "right join JBXX   B ON A.KSH = B.KSH WHERE A.kl=1 and a.zh!=0 and B.XQ_H = " + xqh[0]
            self.cursor.execute(sql)
            result = self.cursor.fetchone()
            result = list(result)
            if None in result:
    ...
    def YSFGK_PROVINCE_TABLE(self):
        ...
        sql = r'select count(a.zh) from kscj  a right join JBXX  b on a.KSH = b.KSH WHERE  a.kl=1 and a.zh!=0'
        self.cursor.execute(sql)
        num = self.cursor.fetchone()[0]  # 总人数
    ...
    def YSFFX_CITY_TABLE(self,dsh):
        ...
        # 地市
        sql = r"select zh,count(zh) from kscj where kl = 1 and yw!=0 and ksh like '" + dsh + r"%' group by zh order by zh desc"
        self.cursor.execute(sql)
        items = self.cursor.fetchall()
        ...
        # 全省
        sql = r"select zh,count(zh) from kscj where kl=1 and zh!=0 group by zh order by zh desc"
        self.cursor.execute(sql)
        items = self.cursor.fetchall()
    ...
    def GSQKFX_PROVINCE(self):
        ...
        writer = pd.ExcelWriter(path + '\\' + "各市情况分析(理科综合).xlsx")
        ...
        sql = "select count(*) as num,avg(zh),stddev_samp(zh) from kscj where zh!=0 and kl=1"
        self.cursor.execute(sql)
        ...
        for ds in dss:
            ...
            sql = r"select count(*) as num,avg(zh),stddev_samp(zh) from kscj where zh!=0 and ksh like '" + ds[
                0] + r"%' and kl=1"
            self.cursor.execute(sql)
            item = self.cursor.fetchone()
            row.append(item[0])
