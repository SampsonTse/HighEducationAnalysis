...
class DTFX:
    ...
    def ZTGK_CITY_TABLE(self, dsh):
        ...
        sql = "select count(wy),AVG(A.wy)   mean,STDDEV_SAMP(A.wy)   std FROM kscj   A where A.kl=1 and a.wy!=0"
        self.cursor.execute(sql)
        result = self.cursor.fetchone()
        result = list(result)
        result.append((float(result[2]) / float(result[1])) * 100)  # 差异系数
    ...
    def YSFGK_PROVINCE_TABLE(self):
        ...
        sql = r'select count(a.wy) from kscj  a right join JBXX  b on a.KSH = b.KSH WHERE  a.kl=1 and a.wy!=0'
    ...
    def DTFX_PROVINCE(self):
        ...
        for idx in idxs:
            ...
            sql = "select sum(kgval),stddev_samp(kgval) from T_GKPJ2020_TKSKGDAMX a right join jbxx b " \
                  "on a.ksh = b.ksh where a.idx=" + str(idx) + " and kmh=101"
        ...
        for xth in xths:
            ...
            sql = "select sum(b.sum),stddev_samp(b.sum) from " \
                  "(select sum(a.xtval) as sum,a.xth,a.ksh from T_GKPJ2020_TSJBNKSXT a " \
                  "right join jbxx on jbxx.ksh=a.ksh where a.kmh = 101 and a.xth = " + str(xth) + " GROUP BY a.ksh,a.xth) b"
        ...
        th = []
        writer = pd.ExcelWriter(path + '\\' + "全省考生答题分析单题区分度难度(英语).xlsx")
        for i in range(len(x)):
            row = []
            row = [str(i + 21), x[i], y[i]]
            self.set_list_precision(row)
    ...
    def MF_LF_CITY_TABLE(self,dsh):
        ...
        for idx in idxs:
            ...
            sql = r"select count(case when amx.kgval=0 then 1 else null end) num1," \
                  ...
                  r"on kscj.ksh=amx.ksh where amx.kmh=101 and amx.idx="+str(idx)+" and amx.ksh like '"+dsh+"%' and kscj.wy!=0"
            self.cursor.execute(sql)
        ...
        for xth in xths:
            ...
            sql = r"select count(case when a.grade=0 then 1 else null end) num1," \
                  ...
                  r"kscj.wy!=0 and sxt.ksh like '"+dsh+r"%' GROUP BY sxt.ksh) a"
    ...
    def MF_LF_PROVINCE_TABLE(self):
        ...
        for xth in xths:
            ...
            sql = r"select count(case when a.grade=0 then 1 else null end) num1," \
                  ...
                  r"(select sxt.ksh,sum(xtval) grade from GKEVA2020.T_GKPJ2020_TSJBNKSXT sxt " \
                  r"right join GKEVA2020.kscj kscj on kscj.ksh=sxt.ksh where sxt.kmh=101 and sxt.xth=" + str(xth) + r" and " \
                  r"kscj.wy!=0  GROUP BY sxt.ksh) a"
            print(sql)
        ...
        df.to_excel(writer, sheet_name="全省单题零分率满分率(英语)", index=None)
        writer.save()
    ...
    def GQXZB_CITY_TABLE(self, dsh):
        ...
        writer = pd.ExcelWriter(path + '\\' + ds_mc + "各区县各分数段分布情况(英语).xlsx")
        ...
        for xqh in xqhs:
            ...
            sql = r"select count(b.wy),avg(b.wy) from (select a.*,rownum rn from " \
                  r"(SELECT KSCJ.* from KSCJ left JOIN JBXX ON KSCJ.KSH = JBXX.KSH " \
                  r"WHERE jbxx.ds_h=" + dsh + " ORDER BY KSCJ.wy DESC) a) b " \
