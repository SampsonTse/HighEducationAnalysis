...
class DTFX:
    ...
    def ZTGK_CITY_TABLE(self, dsh):
        ...
        sql = r"select count(jmx.zf),avg(jmx.zf),STDDEV_SAMP(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
              r"right join GKEVA2020.kscj kscj on kscj.ksh=jmx.ksh where jmx.tzh=3 and kscj.zh!=0 and jmx.kmh = 006 "
        print(sql)
        self.cursor.execute(sql)
        ...
        # 全市
        sql = r"select count(jmx.zf),avg(jmx.zf),STDDEV_SAMP(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
              r"right join GKEVA2020.kscj kscj on kscj.ksh=jmx.ksh where jmx.tzh=3 and kscj.zh!=0  and jmx.kmh = 006 and  jmx.KSH LIKE '" + dsh + "%'"
        self.cursor.execute(sql)
        result = self.cursor.fetchone()
        result = list(result)
        result.append((float(result[2]) / float(result[1])) * 100)  # 差异系数
    ...
    def ZTGK_CITY_IMG(self,dsh):
        ...
        # 全省
        sql = "select count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh = jmx.ksh where jmx.tzh=3 and kmh = 006"
        self.cursor.execute(sql)
        num = self.cursor.fetchone()[0]  # 全省人数

        sql = "select jmx.zf,count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx where right join gkeva2020.kscj kscj on kscj.ksh = jmx.ksh jmx.tzh=3 and kmh = 006  GROUP BY (jmx.zf) ORDER BY jmx.zf desc"
        self.cursor.execute(sql)
        items = list(self.cursor.fetchall())

        province = [None] * 101
        for item in items:
            province[int(item[0])] = round(item[1]/num * 100,2)
        x = list(range(101))
        plt.plot(x, province, color='orange', marker='.', label='全省')

        # 全市
        sql = r"select count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh = jmx.ksh where jmx.tzh=3 and kmh = 006 and ksh like '"+dsh+r"%'"
        self.cursor.execute(sql)
        num = self.cursor.fetchone()[0]  # 全省人数

        sql = "select jmx.zf,count(zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh = jmx.ksh where jmx.tzh=3 " \
    ...
    def YSFGK_PROVINCE_IMG(self):
        ...
        sql = "select jmx.zf,count(zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh = jmx.ksh where jmx.tzh=3 and kmh = 006  GROUP BY (jmx.zf) ORDER BY jmx.zf desc"
        self.cursor.execute(sql)
        items = list(self.cursor.fetchall())
    ...
    def DTFX_CITY_IMG(self, dsh):
        ...
        sql = r"select count(*) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx where jmx.kmh=006 and jmx.tzh=3 and jmx.ksh like '" + dsh + r"%'"
        self.cursor.execute(sql)
        total = self.cursor.fetchone()[0]
        ...
        for idx in idxs:
            ...
            sql = r"select sum(kgval) from GKEVA2020.T_GKPJ2020_TKSKGDAMX amx " \
                  ...
                  r"YMHPT.T_GKPJ2020_TKSTZCJMX jmx where jmx.kmh = 006 and jmx.tzh=3 and " \
    ...
    def YSFFX_CITY_TABLE(self,dsh):
        ...
        sql = r"select jmx.zf,count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
              r"where jmx.kmh=006 and jmx.tzh=3 and zf!=0 GROUP BY jmx.zf ORDER BY jmx.zf desc"
        self.cursor.execute(sql)
        ...
        sql = r"select jmx.zf,count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx where jmx.kmh=006 and jmx.tzh=3 " \
    ...
    def DTFX_CITY_APPENDIX(self, dsh):
        ...
        rows = []
        sql = r"select count(*) from TYMHPT.T_GKPJ2020_TKSTZCJMX where ksh like '"+dsh+r"%' and tzh=3 and kmh=006"
        print(sql)
        self.cursor.execute(sql)
        total = self.cursor.fetchone()[0]
        ...
        for idx in idxs:
            ...
            sql = r"select amx.da,count(amx.da) from GKEVA2020.T_GKPJ2020_TKSKGDAMX amx right join " \
                  ...
                  r" ksh like '"+dsh+r"%' and tzh=3 and kmh=006 ORDER BY zf desc) a) b on amx.ksh=b.ksh " \
    ...
    def GSQKFX_PROVINCE(self):
        ...
        row = []
        # 全省
        sql = r"select count(jmx.zf),avg(zf),STDDEV_SAMP(zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX where kmh=006 and tzh=3 "
        self.cursor.execute(sql)
        ...
        for ds in dss:
            sql = r"select count(jmx.zf),avg(zf),STDDEV_SAMP(zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX " \
                  r"where kmh=006 and tzh=3 and ksh like '" + ds[0] + r"%'"
            self.cursor.execute(sql)
            row = list(self.cursor.fetchone())
            row.append(float(row[2]) / float(row[1]))
            row.insert(1, row[0] / total * 100)
    ...
    def DTFX_PROVINCE(self):
        ...
        sql = r"select count(*) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx where jmx.kmh=006 and jmx.tzh=3 "
        self.cursor.execute(sql)
        total = self.cursor.fetchone()[0]
        ph_num = int(total * 0.27)
    ...
    def YSFGK_PROVINCE_APPENDIX(self):
        ...
        sql = r"select zf,count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX where zf!=0 and kmh=006 and tzh=3 group by zf order by zf desc"
        self.cursor.execute(sql)
    ...
    def DTFX_PROVINCE_APPENDIX(self):
        ...
        rows = []
        sql = r"select count(*) from TYMHPT.T_GKPJ2020_TKSTZCJMX where tzh=3 and kmh=006"
        print(sql)
        self.cursor.execute(sql)
        total = self.cursor.fetchone()[0]
