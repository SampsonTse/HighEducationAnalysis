...
class DTFX:
    ...
    def YSFGK_PROVINCE_IMG(self):
        ...
        plt.ylabel('人数百分比（%）')
        plt.legend(loc='upper center', bbox_to_anchor=(1.05, 1.05))
        plt.savefig(path + '\\全省考生单科成绩分布(地理).png', dpi=1200)
        plt.close()
    ...
    def DTFX_CITY_IMG(self, dsh):
        ...
        plt.savefig(path + '\\各题难度-区分度分布散点图(地理).png', dpi=1200)
        plt.close()
    ...
    def GSQKFX_PROVINCE(self):
        ...
        # 全省
        sql = r"select count(jmx.zf),avg(jmx.zf),STDDEV_SAMP(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
              r"right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh where jmx.kmh=006 and jmx.tzh=3 "
        self.cursor.execute(sql)
        ...
        for ds in dss:
            sql = r"select count(jmx.zf),avg(jmx.zf),STDDEV_SAMP(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
                  r"right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh where jmx.kmh=006 and jmx.tzh=3 and jmx.ksh like '" + ds[0] + r"%'"
            self.cursor.execute(sql)
            row = list(self.cursor.fetchone())
            row.append(float(row[2]) / float(row[1]))
            row.insert(1, row[0] / total * 100)
    ...
    def MF_LF_PROVINCE_TABLE(self):
        ...
        writer = pd.ExcelWriter(path + '\\' + "全省" + "考生答题分析单题分析零分率满分率(地理).xlsx")
        df = pd.DataFrame(data=None, columns=['题号', '零分人数', '零分率', '满分人数', '满分率'])
        ...
        df.to_excel(writer, sheet_name="全省单题零分率满分率(地理)", index=None)
        writer.save()