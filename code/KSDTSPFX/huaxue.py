...
class DTFX:
    ...
    def ZTGK_CITY_TABLE(self,dsh):
        ...
        sql = r"select avg(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
              ...
              r"(jbxx.klsb_h=1 or jbxx.kslb_h=3) and jmx.tzh=6 and jmx.kmh = 005"
        self.cursor.execute(sql)
        result.append(self.cursor.fetchone()[0])
        ...
        sql = r"select avg(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
              r"right join GKEVA2020.jbxx on jbxx.ksh=jmx.ksh where " \
              r"(jbxx.klsb_h=2 or jbxx.kslb_h=4) and jmx.kmh = 005 and jmx.tzh=6"
        self.cursor.execute(sql)
        result.append(self.cursor.fetchone()[0])
        ...
        sql = r"select avg(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
              r"right join GKEVA2020.jbxx on jbxx.ksh=jmx.ksh where " \
              r"(jbxx.klsb_h=1 or jbxx.kslb_h=2) and jmx.kmh = 005 and jmx.tzh=6"
        self.cursor.execute(sql)
        result.append(self.cursor.fetchone()[0])
        ...
        sql = r"select avg(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
              r"right join GKEVA2020.jbxx on jbxx.ksh=jmx.ksh where (jbxx.klsb_h=3 or jbxx.kslb_h=4) and jmx.kmh = 005 and jmx.tzh=6"
        self.cursor.execute(sql)
        result.append(self.cursor.fetchone()[0])
        ...
        df.to_excel(writer,sheet_name="各类别考生成绩比较(化学)")
        ...
        for xqh in xqhs:
            result = []
            sql = r"select count(jmx.zf),avg(jmx.zf),STDDEV_SAMP(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
                  r"where jmx.tzh=6 and kmh = 005 and  KSH LIKE '" + xqh + r"%'"
            self.cursor.execute(sql)
            print(sql)
            result = self.cursor.fetchone()
            result = list(result)
    ...
    def YSFGK_PROVICNE_TABLE(self):
        ...
        writer = pd.ExcelWriter(path + '\\' + "考生答题分析总体概括(化学).xlsx")
    ...
    def DTFX_CITY_TABLE(self,dsh):
        ...
        df.to_excel(writer,sheet_name="地市考生单题作答情况(化学)")
        writer.save()