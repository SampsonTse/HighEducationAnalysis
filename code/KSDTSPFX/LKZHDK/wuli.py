...
class DTFX:
    ...
    def ZTGK_CITY_TABLE(self, dsh):
        ...
        writer = pd.ExcelWriter(path + '\\' + ds_mc + "考生答题分析总体概括(物理).xlsx")
        ...
        df.to_excel(writer, sheet_name="各类别考生成绩比较(物理)", index=None)
        ...
        sql = r"select count(jmx.zf),avg(jmx.zf),STDDEV_SAMP(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
              r"right join GKEVA2020.kscj kscj on kscj.ksh=jmx.ksh where jmx.tzh=9 and kscj.zh!=0 and jmx.kmh = 005 and jmx.zf!=0"
        self.cursor.execute(sql)
        ...
        df.to_excel(excel_writer=writer, sheet_name="各县区考生成绩比较(物理)", index=None)
        writer.save()
    ...
    def ZTGK_CITY_IMG(self, dsh):
        ...
        # 全省
        sql = "select count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh = jmx.ksh where jmx.tzh=9 and jmx.kmh = 005"
        
        self.cursor.execute(sql)
        num = self.cursor.fetchone()[0]  # 全省人数

        sql = "select jmx.zf,count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh = jmx.ksh where jmx.tzh=9 and jmx.kmh = 005  GROUP BY (jmx.zf) ORDER BY jmx.zf desc"
        self.cursor.execute(sql)
        ...
        # 全市
        sql = r"select count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh = jmx.ksh where jmx.tzh=9 and jmx.kmh = 005 and jmx.ksh like '" + dsh + r"%'"
        self.cursor.execute(sql)
        num = self.cursor.fetchone()[0]  # 全省人数
        ...
        plt.savefig(path + '\\地市及全省考生单科成绩分布(物理).png', dpi=1200)
        plt.close()
    ...
    def YSFGK_PROVINCE_IMG(self):
        ...
        # 全省
        sql = "select count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx left join gkeva2020.kscj kscj on kscj.ksh = jmx.ksh where jmx.tzh=9 and jmx.kmh = 005"
        
        self.cursor.execute(sql)
        num = self.cursor.fetchone()[0]  # 全省人数

        sql = "select jmx.zf,count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx left join gkeva2020.kscj kscj on kscj.ksh = jmx.ksh where jmx.tzh=9 and jmx.kmh = 005  GROUP BY (jmx.zf) ORDER BY jmx.zf desc"
        self.cursor.execute(sql)
        items = list(self.cursor.fetchall())
        ...
        plt.ylabel('人数百分比（%）')
        plt.legend(loc='upper center', bbox_to_anchor=(1.05, 1.05))
        plt.savefig(path + '\\全省考生单科成绩分布(物理).png', dpi=1200)
        plt.close()
    ...
    def YSFGK_PROVICNE_TABLE(self):
        ...
        writer = pd.ExcelWriter(path + '\\' + "全省考生答题分析原始分概括(物理).xlsx")
    ...
    def DTFX_CITY_TABLE(self,dsh):
        ...
        writer = pd.ExcelWriter(path + '\\' + ds_mc + "考生答题分析单题分析(物理).xlsx")
        ...
        for dth in dths:
            ...
            sql = r"select avg(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh where jmx.kmh=005 and jmx.tzh="+str(dth)+" and ksh like '"+dsh+r"%'"
            
            self.cursor.execute(sql)
            row.append(self.cursor.fetchone()[0])

            sql = r"select avg(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh where jmx.kmh=005 and jmx.tzh="+str(dth)
            ...
            sql = r"select avg(c.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX c right join" \
                  ...
                  r" right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh where jmx.kmh=005 and jmx.ksh like '"+dsh+"%' and jmx.tzh=9 ORDER BY jmx.zf desc) a) b " \
        ...
        df.to_excel(writer,sheet_name="地市考生单题作答情况(物理)",index=None)
        writer.save()
    ...
    def DTFX_CITY_IMG(self, dsh):
        ...
        sql = r"select count(*) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh" \
              r" where jmx.kmh=005 and jmx.tzh=9 and jmx.ksh like '" + dsh + r"%'"
        self.cursor.execute(sql)
        total = self.cursor.fetchone()[0]
        ...
        for idx in idxs:
            ...
            sql = r"select sum(kgval) from GKEVA2020.T_GKPJ2020_TKSKGDAMX amx " \
                  ...
                  r"YMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh where jmx.kmh = 005 and jmx.tzh=9 and " \
        ...
        for dth in dths:
            ...
            sql = r"select a.zf,b.zf,b.ksh from TYMHPT.T_GKPJ2020_TKSTZCJMX a right join " \
                  ...
                  r"jmx.kmh=005 and jmx.tzh=9 and jmx.ksh like '" + dsh + r"%') b on a.ksh=b.ksh where a.kmh=005 and a.tzh=" + str(dth)
            self.cursor.execute(sql)
        ...
        plt.savefig(path + '\\各题难度-区分度分布散点图(物理).png', dpi=1200)
        plt.close()
    ...
    def YSFFX_CITY_TABLE(self,dsh):
        ...
        writer = pd.ExcelWriter(path + '\\' + ds_mc + "考生答题水平分析原始分概括(物理).xlsx")
        ...
        sql = r"select jmx.zf,count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
              r" right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh where jmx.kmh=005 and jmx.tzh=9 and jmx.zf!=0 GROUP BY jmx.zf ORDER BY jmx.zf desc"
        self.cursor.execute(sql)
        ...
        sql = r"select jmx.zf,count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh where jmx.kmh=005 and jmx.tzh=9 " \
        ...
        df.to_excel(excel_writer=writer, sheet_name='地市及全省考生一分段概括(物理)', index=None)
    ...
    def DTFX_CITY_APPENDIX(self, dsh):
        ...
        rows = []
        sql = r"select count(*) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh" \
              r" where jmx.ksh like '"+dsh+r"%' and jmx.tzh=9 and jmx.kmh=005"
        ...
        for idx in idxs:
            ...
            sql = r"select amx.da,count(amx.da) from GKEVA2020.T_GKPJ2020_TKSKGDAMX amx right join " \
                  ...
                  r" jmx.ksh like '"+dsh+r"%' and jmx.tzh=9 and jmx.kmh=005 ORDER BY jmx.zf desc) a) b on amx.ksh=b.ksh " \
            ...
            sql = r"select amx.da,count(amx.da) from GKEVA2020.T_GKPJ2020_TKSKGDAMX amx right join " \
                  ...
                  r"on kscj.ksh=jmx.ksh where jmx.ksh like '" + dsh + r"%' and jmx.tzh=9 and jmx.kmh=005 ORDER BY jmx.zf desc) a) b on amx.ksh=b.ksh " \
    ...
    def GSQKFX_PROVINCE(self):
        ...
        writer = pd.ExcelWriter(path + '\\' + "各市情况分析(物理).xlsx")
        ...
        df.to_excel(writer, index=None, sheet_name="各市考生成绩比较(物理)")
        writer.save()
    ...
    def DTFX_PROVINCE(self):
        ...
        for idx in idxs:
            ...
            sql = r"select sum(kgval) from GKEVA2020.T_GKPJ2020_TKSKGDAMX amx " \
                  ...
                  r"YMHPT.T_GKPJ2020_TKSTZCJMX jmx where jmx.kmh = 005 and jmx.tzh=9 " \
        ...
        for dth in dths:
            ...
            sql = r"select a.zf,b.zf,b.ksh from TYMHPT.T_GKPJ2020_TKSTZCJMX a right join " \
                  r"(select jmx.ksh,jmx.zf from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh where " \
                  r"jmx.kmh=005 and jmx.tzh=9) b on a.ksh=b.ksh where a.kmh=005 and a.tzh="+str(dth)
            self.cursor.execute(sql)
            result = np.array(self.cursor.fetchall(), dtype="float64")
        ...
        df.to_excel(writer,index=None,sheet_name="考生单题作答情况(物理)")
        writer.save()
    ...
    def YSFGK_PROVINCE_APPENDIX(self):
        ...
        writer = pd.ExcelWriter(path + '\\' + "原始分概括(物理).xlsx")
        ...
        sql = r"select jmx.zf,count(jmx.zf) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh" \
              r" where jmx.zf!=0 and jmx.kmh=005 and jmx.tzh=9 group by jmx.zf order by jmx.zf desc"
        ...
        df.to_excel(writer, index=None, sheet_name="全省考生一分段(物理)")
    ...
    def DTFX_PROVINCE_APPENDIX(self):
        ...
        writer = pd.ExcelWriter(path + '\\' + "考生答题分析单题分析(物理).xlsx")

        rows = []
        sql = r"select count(*) from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx right join " \
              r"gkeva2020.kscj kscj on kscj.ksh=jmx.ksh where  jmx.tzh=9 and jmx.kmh=005"
        ...
        for idx in idxs:
            ...
            sql = r"select amx.da,count(amx.da) from GKEVA2020.T_GKPJ2020_TKSKGDAMX amx right join " \
                  ...
                  r"  jmx.tzh=9 and jmx.kmh=005 ORDER BY jmx.zf desc) a) b on amx.ksh=b.ksh " \
                  r"where b.rn BETWEEN 1 and "+str(low)+r" and amx.kmh=005 and amx.idx="+str(idx)+" GROUP BY amx.da"
            ...
            sql = r"select amx.da,count(amx.da) from GKEVA2020.T_GKPJ2020_TKSKGDAMX amx right join " \
                  r"(select a.*,rownum rn from (select jmx.ksh,jmx.zf from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
                  r"right join gkeva2020.kscj kscj on kscj.ksh=jmx.ksh where" \
                  r"  jmx.tzh=9 and jmx.kmh=005 ORDER BY zf desc) a) b on amx.ksh=b.ksh " \
                  r"where b.rn BETWEEN "+str(low+1)+r" and " + str(high) + r" and amx.kmh=005 and amx.idx=" + str(idx) + " GROUP BY amx.da"
            self.cursor.execute(sql)
            items = []
            items = self.cursor.fetchall()
    ...
    def MF_LF_CITY_TABLE(self,dsh):
        ...
        idxs1 = list(range(14, 19))
        idxs2 = list(range(19,22))
        dths = [22, 23, 24, 25, 33, 34]
        txt = idxs1 + idxs2 + dths
    ...
    def JGFX_CITY_TABLE(self,dsh):
        ...
        writer = pd.ExcelWriter(path + '\\' + ds_mc + "考生答题分析结构分析(物理).xlsx")

        df1 = pd.read_excel(path+"\\"+ds_mc+"考生答题分析单题分析(物理).xlsx",sheet_name=0)
        ...
        # num = 30.00
        # row.append(mean_city[0]+mean_city[1]+mean_city[2]+mean_city[3]+mean_city[4])
        #
        ...
        # num = 18.00
        # row.append(mean_city[5] + mean_city[6] + mean_city[7] )
        #
        # sql = r"select stddev_samp(a.score) from (SELECT sum(amx.kgval) score from " \
        #       r"GKEVA2020.T_GKPJ2020_TKSKGDAMX amx right join GKEVA2020.kscj kscj on amx.ksh=kscj.ksh where" \
        ...
        # num = 15.00
        # row.append(mean_city[8] + mean_city[9] )
        ...
        # num = 32.00
        # row.append(mean_city[10] + mean_city[11])
        # sql = r"select stddev_samp(a.score) from " \
        #       r"(SELECT sum(jmx.zf) score from TYMHPT.T_GKPJ2020_TKSTZCJMX jmx " \
        ...
        df2 = pd.DataFrame(columns=['知识板块', '题号', '分值', '平均分', '标准差', '差异系数', '得分率', '全省平均分'])

        row = ["力学", "14、15、16、20、23、24", "45.00"]
        num = 45.00
        row.append(mean_city[0]+mean_city[1]+mean_city[2]+mean_city[6]+mean_city[9]+mean_city[10])
        sql = r"select STDDEV_SAMP(a.score+b.score) from (select amx.ksh,sum(amx.kgval) score " \
              ...
              r"TYMHPT.T_GKPJ2020_TKSTZCJMX jmx where jmx.ksh like '01%' and jmx.kmh=005 and jmx.tzh in (23,24) GROUP BY jmx.ksh) b on a.ksh=b.ksh"
        self.cursor.execute(sql)
        row.append(self.cursor.fetchone()[0])
        row.append(row[-1] / row[-2] * 100)
        row.append(row[-3] / num)
        ...
        row = ["电学", "17、22", "12.00"]
        num = 12.00
        row.append(mean_city[3] + mean_city[8] )
        sql = r"select STDDEV_SAMP(a.score+b.score) from (select amx.ksh,sum(amx.kgval) score " \
              r"from GKEVA2020.T_GKPJ2020_TKSKGDAMX amx right join GKEVA2020.kscj kscj " \
              r"on kscj.ksh=amx.ksh where amx.ksh like '" + dsh + r"%' and  amx.kmh=005 and amx.idx " \
              r"in (17) GROUP BY amx.ksh) a left join (select jmx.ksh,sum(jmx.zf) score from " \
              r"TYMHPT.T_GKPJ2020_TKSTZCJMX jmx where jmx.ksh like '01%' and jmx.kmh=005 and jmx.tzh in (22) GROUP BY jmx.ksh) b on a.ksh=b.ksh"
        self.cursor.execute(sql)
        row.append(self.cursor.fetchone()[0])
        row.append(row[-1] / row[-2] * 100)
        row.append(row[-3] / num)
